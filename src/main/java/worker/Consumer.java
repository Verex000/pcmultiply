...
public class Consumer extends Thread implements Runnable {
	...
	public Consumer(Buffer buffer, Counter counter, Counter multCounter, SumCounter sumCounter, int threadName,
			ReentrantLock lock) {
	...
	public void run() {
		...
		while (sharedConsumerCounter.get() < PCMatrix.MATRICIES) {
			...
			try {
//				System.out.println("counter "+ sharedConsumerCounter.get());
...
//				System.out.println("counter after"+ sharedConsumerCounter.get());
//				int total = sharedConsumerCounter.get();
